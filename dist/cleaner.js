 ...
 */
export function parseAndClean(html) {
...
function consolidateElements(elements) {
    ...
    for (const el of elements) {
        if (!el.content && el.type !== 'break')
            continue;
        const last = result[result.length - 1];
        // Skip consecutive breaks
        if (el.type === 'break' && last?.type === 'break')
            continue;
    ...
    while (result.length && result[0].type === 'break')
        result.shift();
    while (result.length && result[result.length - 1].type === 'break')
        result.pop();
    return result;
}
/**
