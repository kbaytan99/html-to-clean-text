 ...
 */
function findBreakPoint(text, maxPos) {
 ...
 */
export function chunkText(text, targetTokens = 1000) {
    const chunks = [];
    // Normalize text
...
export function detectLanguage(text) {
    ...
    if (/\b(der|die|das|und|ist|für)\b/.test(lowerSample))
        return 'de';
    if (/\b(le|la|les|et|est|pour)\b/.test(lowerSample))
        return 'fr';
    if (/\b(el|la|los|las|es|para)\b/.test(lowerSample))
        return 'es';
    if (/\b(il|la|gli|le|è|per)\b/.test(lowerSample))
        return 'it';
    if (/\b(o|a|os|as|é|para)\b/.test(lowerSample))
        return 'pt';
    if (/\b(de|het|een|en|is|voor)\b/.test(lowerSample))
        return 'nl';
    // Default to English
    return 'en';
}
 ...
 */
export function toJSON(text, targetTokens = 1000) {
